...
1. `bash "${CLAUDE_PLUGIN_ROOT}/hooks/clip.sh" <숫자/범위>`를 실행하여 이미지 경로를 얻는다.
...
1. 숫자/범위 부분으로 `bash "${CLAUDE_PLUGIN_ROOT}/hooks/clip.sh" <숫자/범위>`를 실행한다.
2. 실패 시 에러 메시지를 전달하고 중단한다.
3. 성공 시 반환된 각 경로의 이미지를 Read 도구로 모두 읽는다.
4. 텍스트 부분의 요청에 맞춰 이미지를 분석하고 한국어로 응답한다.

## 입력이 텍스트만 있는 경우 (예: "이 에러 뭐야")
1. `bash "${CLAUDE_PLUGIN_ROOT}/hooks/clip.sh" latest`를 실행하여 최신 이미지 경로를 얻는다.
2. 이미지를 Read 도구로 읽는다.
...
## 목록 표시 개수 변경
입력이 "set-max"인 경우:
- `bash "${CLAUDE_PLUGIN_ROOT}/hooks/clip.sh" set-max <숫자>`를 실행한다.
